 ...
 * Result 12 The result may contain fives. ;-) The start number will always be
 * smaller than the end number. Both numbers can be also negative!
 * 
...
public class Kata {
	public static long dontGiveMeFive(int start, int end) {
		...
		// }
		// return count;
		return IntStream.rangeClosed(start, end).filter(x -> !String.valueOf(x).contains("5")).count();
	}
}